...
// Simplified AI analysis function using only mock data (no external API dependencies)
const analyzeMealImage = async (imagePath) => {
  try {
    console.log('Starting mock AI analysis for image:', imagePath);

    // Generate mock analysis since we're using mock data only
...
const getMockFoodDetection = () => {
  const mockFoods = [
    ...
    { name: 'Avocado', confidence: 0.86, type: 'mock' },
    { name: 'Eggs', confidence: 0.91, type: 'mock' },
    { name: 'Whole Wheat Bread', confidence: 0.84, type: 'mock' },
    { name: 'Banana', confidence: 0.88, type: 'mock' }
  ];
...
const getMockNutritionData = (foodName) => {
  const mockNutritionDatabase = {
    ...
    'Pasta': { calories: 131, protein: 5, carbs: 25, fat: 1.1, fiber: 1.5, sugar: 1, sodium: 1 },
    'Cheese': { calories: 113, protein: 7, carbs: 1.3, fat: 9.3, fiber: 0, sugar: 0.1, sodium: 174 },
